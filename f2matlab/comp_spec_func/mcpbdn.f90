...
INTEGER, INTENT(IN)                      :: n
COMPLEX, INTENT(IN)                      :: z
COMPLEX, INTENT(OUT)                     :: cpb(0:100)
COMPLEX, INTENT(OUT)                     :: cpd(0:100)
...
ELSE
  ...
  ELSE
    ...
    DO  m=1,150
      ...
      END DO
      cdw=(-1)**m*gm*zq**m/fm
      IF (m == 2*INT(m/2)) cdn1=cdn1+cdw
      IF (m /= 2*INT(m/2)) cdn2=cdn2+cdw
      IF (CDABS(cdw) < CDABS(cdn1)*eps) EXIT
      IF (CDABS(cdw) < CDABS(cdn2)*eps) EXIT
    END DO
    20       cdn=cb0*(cdn0+cdn1+cdn2)
  END IF
END IF
RETURN
