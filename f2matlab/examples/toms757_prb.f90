...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TOMS757_PRB:'
  write ( *, '(a)' ) '  Test the uncommon special function routines in TOMS757.'
...
!
  ...
  real ( kind = 8 ), dimension(0:8) :: ab0f = (/ &
    real ( -0.68121927093549469816d0, kind = 8 ), &
    real ( -0.78867919816149252495d0, kind = 8 ), &
    real (  0.5121581776818819543d-1, kind = 8 ), &
    real ( -0.71092352894541296d-3, kind = 8 ), &
    real (  0.368681808504287d-5, kind = 8 ), &
    real ( -0.917832337237d-8, kind = 8 ), &
    real (  0.1270202563d-10, kind = 8 ), &
    real ( -0.1076888d-13, kind = 8 ), &
    real (  0.599d-17, kind = 8 ) /)
  real ( kind = 8 ), dimension(0:8) :: ab0g = (/ &
    real ( -0.60506039430868273190d0, kind = 8 ),  &
    real ( -0.41950398163201779803d0, kind = 8 ),  &
    real (  0.1703265125190370333d-1, kind = 8 ),  &
    real ( -0.16938917842491397d-3, kind = 8 ),  &
    real (  0.67638089519710d-6, kind = 8 ),  &
    real ( -0.135723636255d-8, kind = 8 ),  &
    real (  0.156297065d-11, kind = 8 ),  &
    real ( -0.112887d-14, kind = 8 ),  &
    real (  0.55d-18, kind = 8 ) /)
  real ( kind = 8 ), dimension(0:8) :: ab0h = (/ &
    real (  1.38202655230574989705d0, kind = 8 ),  &
    real ( -0.30097929073974904355d0, kind = 8 ),  &
    real (  0.794288809364887241d-2, kind = 8 ),  &
    real ( -0.6431910276847563d-4, kind = 8 ),  &
    real (  0.22549830684374d-6, kind = 8 ),  &
    real ( -0.41220966195d-9, kind = 8 ),  &
    real (  0.44185282d-12, kind = 8 ),  &
    real ( -0.30123d-15, kind = 8 ),  &
    real (  0.14d-18, kind = 8 )  /)
  real ( kind = 8 ), dimension(0:27) :: ab0as = (/ &
    real (  1.97755499723693067407d+0, kind = 8 ),  &
    real ( -0.1046024792004819485d-1, kind = 8 ),  &
    real (  0.69680790253625366d-3, kind = 8 ),  &
    real ( -0.5898298299996599d-4, kind = 8 ),  &
    real (  0.577164455305320d-5, kind = 8 ),  &
    real ( -0.61523013365756d-6, kind = 8 ),  &
    real (  0.6785396884767d-7, kind = 8 ),  &
    real ( -0.723062537907d-8, kind = 8 ),  &
    real (  0.63306627365d-9, kind = 8 ),  &
    real ( -0.989453793d-11, kind = 8 ),  &
    real ( -0.1681980530d-10, kind = 8 ),  &
    real (  0.673799551d-11, kind = 8 ),  &
    real ( -0.200997939d-11, kind = 8 ),  &
    real (  0.54055903d-12, kind = 8 ),  &
    real ( -0.13816679d-12, kind = 8 ),  &
    real (  0.3422205d-13, kind = 8 ),  &
    real ( -0.826686d-14, kind = 8 ),  &
    real (  0.194566d-14, kind = 8 ),  &
    real ( -0.44268d-15, kind = 8 ),  &
    real (  0.9562d-16, kind = 8 ),  &
    real ( -0.1883d-16, kind = 8 ),  &
    real (  0.301d-17, kind = 8 ),  &
    real ( -0.19d-18, kind = 8 ),  &
    real ( -0.14d-18, kind = 8 ),  &
    real (  0.11d-18, kind = 8 ),  &
    real ( -0.4d-19, kind = 8 ),  &
    real (  0.2d-19, kind = 8 ),  &
    real ( -0.1d-19, kind = 8 ) /)
  real ( kind = 8 ) abram0
...
!
  ...
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    ...
    real ( 0.84721859650456925922D+00, kind = 8 ), &
    real ( 0.77288934483988301615D+00, kind = 8 ), &
    real ( 0.59684345853450151603D+00, kind = 8 ), &
    real ( 0.29871735283675888392D+00, kind = 8 ), &
    real ( 0.15004596450516388138D+00, kind = 8 ), &
    real ( 0.11114662419157955096D+00, kind = 8 ), &
    real ( 0.83909567153151897766D-01, kind = 8 ), &
    real ( 0.56552321717943417515D-01, kind = 8 ), &
    real ( 0.49876496603033790206D-01, kind = 8 ), &
    real ( 0.44100889219762791328D-01, kind = 8 ), &
    real ( 0.19738535180254062496D-01, kind = 8 ), &
    real ( 0.86193088287161479900D-02, kind = 8 ), &
    real ( 0.40224788162540127227D-02, kind = 8 ), &
    real ( 0.19718658458164884826D-02, kind = 8 ), &
    real ( 0.10045868340133538505D-02, kind = 8 ), &
    real ( 0.15726917263304498649D-03, kind = 8 ), &
    real ( 0.10352666912350263437D-04, kind = 8 ), &
    real ( 0.91229759190956745069E-06, kind = 8 ), &
    real ( 0.25628287737952698742E-09, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real ( 0.0019531250D+00, kind = 8 ), &
    real ( 0.0078125000D+00, kind = 8 ), &
    real ( 0.0312500000D+00, kind = 8 ), &
    real ( 0.1250000000D+00, kind = 8 ), &
    real ( 0.5000000000D+00, kind = 8 ), &
    real ( 1.0000000000D+00, kind = 8 ), &
    real ( 1.2500000000D+00, kind = 8 ), &
    real ( 1.5000000000D+00, kind = 8 ), &
    real ( 1.8750000000D+00, kind = 8 ), &
    real ( 2.0000000000D+00, kind = 8 ), &
    real ( 2.1250000000D+00, kind = 8 ), &
    real ( 3.0000000000D+00, kind = 8 ), &
    real ( 4.0000000000D+00, kind = 8 ), &
    real ( 5.0000000000D+00, kind = 8 ), &
    real ( 6.0000000000D+00, kind = 8 ), &
    real ( 7.0000000000D+00, kind = 8 ), &
    real ( 10.0000000000D+00, kind = 8 ), &
    real ( 15.0000000000D+00, kind = 8 ), &
    real ( 20.0000000000D+00, kind = 8 ), &
    real ( 40.0000000000D+00, kind = 8 ) /)
...
!
  ...
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.49828219848799921792D+00, kind = 8 ), &
    real ( 0.49324391773047288556D+00, kind = 8 ), &
    real ( 0.47431612784691234649D+00, kind = 8 ), &
    real ( 0.41095983258760410149D+00, kind = 8 ), &
    real ( 0.25317617388227035867D+00, kind = 8 ), &
    real ( 0.14656338138597777543D+00, kind = 8 ), &
    real ( 0.11421547056018366587D+00, kind = 8 ), &
    real ( 0.90026307383483764795D-01, kind = 8 ), &
    real ( 0.64088214170742303375D-01, kind = 8 ), &
    real ( 0.57446614314166191085D-01, kind = 8 ), &
    real ( 0.51581624564800730959D-01, kind = 8 ), &
    real ( 0.25263719555776416016D-01, kind = 8 ), &
    real ( 0.11930803330196594536D-01, kind = 8 ), &
    real ( 0.59270542280915272465D-02, kind = 8 ), &
    real ( 0.30609215358017829567D-02, kind = 8 ), &
    real ( 0.16307382136979552833D-02, kind = 8 ), &
    real ( 0.28371851916959455295D-03, kind = 8 ), &
    real ( 0.21122150121323238154D-04, kind = 8 ), &
    real ( 0.20344578892601627337D-05, kind = 8 ), &
    real ( 0.71116517236209642290E-09, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real ( 0.0019531250D+00, kind = 8 ), &
...
!
  ...
  real ( kind = 8 ), parameter :: six = real ( 6.0D+00, kind = 8 )
  real ( kind = 8 ), parameter :: three = 3.0D+00
  real ( kind = 8 ), parameter :: two = 2.0D+00
  real ( kind = 8 ) x
  real ( kind = 8 ) xvalue
...
!
  ...
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.44213858162107913430D+00, kind = 8 ), &
    real ( 0.43923379545684026308D+00, kind = 8 ), &
    real ( 0.42789857297092602234D+00, kind = 8 ), &
    real ( 0.38652825661854504406D+00, kind = 8 ), &
    real ( 0.26538204413231368110D+00, kind = 8 ), &
    real ( 0.16848734838334595000D+00, kind = 8 ), &
    real ( 0.13609200032513227112D+00, kind = 8 ), &
    real ( 0.11070330027727917352D+00, kind = 8 ), &
    real ( 0.82126019995530382267D-01, kind = 8 ), &
    real ( 0.74538781999594581763D-01, kind = 8 ), &
    real ( 0.67732034377612811390D-01, kind = 8 ), &
    real ( 0.35641808698811851022D-01, kind = 8 ), &
    real ( 0.17956589956618269083D-01, kind = 8 ), &
    real ( 0.94058737143575370625D-02, kind = 8 ), &
    real ( 0.50809356204299213556D-02, kind = 8 ), &
    real ( 0.28149565414209719359D-02, kind = 8 ), &
    real ( 0.53808696422559303431D-03, kind = 8 ), &
    real ( 0.44821756380146327259D-04, kind = 8 ), &
    real ( 0.46890678427324100410D-05, kind = 8 ), &
    real ( 0.20161544850996420504D-08, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real ( 0.0019531250D+00, kind = 8 ), &
...
!
  ...
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( -0.75228838916610124300D+00, kind = 8 ), &
    real ( -0.57348350185854889466D+00, kind = 8 ), &
    real ( -0.76569840313421291743D+00, kind = 8 ), &
    real ( -0.65181015505382467421D+00, kind = 8 ), &
    real ( -0.55881974894471876922D+00, kind = 8 ), &
    real ( -0.56902352870716815309D+00, kind = 8 ), &
    real ( -0.47800749642926168100D+00, kind = 8 ), &
    real ( -0.46567398346706861416D+00, kind = 8 ), &
    real ( -0.96783140945618013679D-01, kind = 8 ), &
    real ( -0.34683049857035607494D-03, kind = 8 ), &
    real (  0.34658366917927930790D-03, kind = 8 ), &
    real (  0.27657581846051227124D-02, kind = 8 ), &
    real (  0.14595330491185717833D+00, kind = 8 ), &
    real (  0.23631734191710977960D+00, kind = 8 ), &
    real (  0.33289264538612212697D+00, kind = 8 ), &
    real (  0.33318759129779422976D+00, kind = 8 ), &
    real (  0.33332945170523851439D+00, kind = 8 ), &
    real (  0.33333331724248357420D+00, kind = 8 ), &
    real (  0.33333333329916901594D+00, kind = 8 ), &
    real (  0.33333333333329380187D+00, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real ( -12.0000000000D+00, kind = 8 ), &
    real ( -11.0000000000D+00, kind = 8 ), &
    real ( -10.0000000000D+00, kind = 8 ), &
    real (  -9.5000000000D+00, kind = 8 ), &
    real (  -9.0000000000D+00, kind = 8 ), &
    real (  -6.5000000000D+00, kind = 8 ), &
    real (  -4.0000000000D+00, kind = 8 ), &
    real (  -1.0000000000D+00, kind = 8 ), &
    real (  -0.2500000000D+00, kind = 8 ), &
    real (  -0.0009765625D+00, kind = 8 ), &
    real (   0.0009765625D+00, kind = 8 ), &
    real (   0.0078125000D+00, kind = 8 ), &
    real (   0.5000000000D+00, kind = 8 ), &
    real (   1.0000000000D+00, kind = 8 ), &
    real (   4.0000000000D+00, kind = 8 ), &
    real (   4.5000000000D+00, kind = 8 ), &
    real (   6.0000000000D+00, kind = 8 ), &
    real (   8.0000000000D+00, kind = 8 ), &
    real (  10.0000000000D+00, kind = 8 ), &
    real (  12.0000000000D+00, kind = 8 ) /)         
...
!
  ...
  real ( kind = 8 ) airy_bi_int
  real ( kind = 8 ) cheval
  real ( kind = 8 ), parameter :: eight = real ( 8.0, kind = 8 )
  real ( kind = 8 ), parameter :: four = real ( 4.0D+00, kind = 8 )
...
!
  ...
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.17660819031554631869D-01, kind = 8 ), &
    real ( -0.15040424806140020451D-01, kind = 8 ), &
    real (  0.14756446293227661920D-01, kind = 8 ), &
    real ( -0.11847304264848446271D+00, kind = 8 ), &
    real ( -0.64916741266165856037D-01, kind = 8 ), &
    real (  0.97260832464381044540D-01, kind = 8 ), &
    real (  0.50760058495287539119D-01, kind = 8 ), &
    real ( -0.37300500963429492179D+00, kind = 8 ), &
    real ( -0.13962988442666578531D+00, kind = 8 ), &
    real ( -0.12001735266723296160D-02, kind = 8 ), &
    real (  0.12018836117890354598D-02, kind = 8 ), &
    real (  0.36533846550952011043D+00, kind = 8 ), &
    real (  0.87276911673800812196D+00, kind = 8 ), &
    real (  0.48219475263803429675D+02, kind = 8 ), &
    real (  0.44006525804904178439D+06, kind = 8 ), &
    real (  0.17608153976228301458D+07, kind = 8 ), &
    real (  0.73779211705220007228D+07, kind = 8 ), &
    real (  0.14780980310740671617D+09, kind = 8 ), &
    real (  0.97037614223613433849D+11, kind = 8 ), &
    real (  0.11632737638809878460D+15, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real ( -12.0000000000D+00, kind = 8 ), &
    ...
    real (   4.0000000000D+00, kind = 8 ), &
    real (   8.0000000000D+00, kind = 8 ), &
    real (   8.5000000000D+00, kind = 8 ), &
    real (   9.0000000000D+00, kind = 8 ), &
    real (  10.0000000000D+00, kind = 8 ), &
    real (  12.0000000000D+00, kind = 8 ), &
    real (  14.0000000000D+00, kind = 8 ) /)         
...
!
  ...
  integer nterm5
  integer nterm6
  real ( kind = 8 ), parameter :: one = real ( 1.0, kind = 8 )
  real ( kind = 8 ), parameter :: three = 3.0D+00
  real ( kind = 8 ) x
  real ( kind = 8 ) xvalue
  real ( kind = 8 ), parameter :: zero = 0.0D+00
...
!
  ...
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.20468308070040542435D+00, kind = 8 ), &
    real (  0.18374662832557904078D+00, kind = 8 ), &
    real ( -0.11667221729601528265D+00, kind = 8 ), &
    real (  0.31466934902729557596D+00, kind = 8 ), &
    real ( -0.37089040722426257729D+00, kind = 8 ), &
    real ( -0.25293059772424019694D+00, kind = 8 ), &
    real (  0.28967410658692701936D+00, kind = 8 ), &
    real ( -0.34644836492634090590D+00, kind = 8 ), &
    real (  0.28076035913873049496D+00, kind = 8 ), &
    real (  0.21814994508094865815D+00, kind = 8 ), &
    real (  0.20526679000810503329D+00, kind = 8 ), &
    real (  0.22123695363784773258D+00, kind = 8 ), &
    real (  0.23521843981043793760D+00, kind = 8 ), &
    real (  0.82834303363768729338D-01, kind = 8 ), &
    real (  0.45757385490989281893D-01, kind = 8 ), &
    real (  0.44150012014605159922D-01, kind = 8 ), &
    real (  0.39951133719508907541D-01, kind = 8 ), &
    real (  0.35467706833949671483D-01, kind = 8 ), &
    real (  0.31896005100679587981D-01, kind = 8 ), &
    real (  0.26556892713512410405D-01, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (  -0.0019531250D+00, kind = 8 ), &
    ...
    real (  -4.0000000000D+00, kind = 8 ), &
    real (  -8.0000000000D+00, kind = 8 ), &
    real (  -8.2500000000D+00, kind = 8 ), &
    real (  -9.0000000000D+00, kind = 8 ), &
    real ( -10.0000000000D+00, kind = 8 ), &
    real ( -11.0000000000D+00, kind = 8 ), &
    real ( -13.0000000000D+00, kind = 8 ), &
    real (   0.0019531250D+00, kind = 8 ), &
    real (   0.1250000000D+00, kind = 8 ), &
    real (   1.0000000000D+00, kind = 8 ), &
    real (   4.0000000000D+00, kind = 8 ), &
    real (   7.0000000000D+00, kind = 8 ), &
    real (   7.2500000000D+00, kind = 8 ), &
    real (   8.0000000000D+00, kind = 8 ), &
    real (   9.0000000000D+00, kind = 8 ), &
    real (  10.0000000000D+00, kind = 8 ), &
    real (  12.0000000000D+00, kind = 8 ) /)       
...
!
  ...
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.40936798278458884024D+00, kind = 8 ), &
    real ( 0.37495291608048868619D+00, kind = 8 ), &
    real ( 0.22066960679295989454D+00, kind = 8 ), &
    real ( 0.77565356679703713590D-01, kind = 8 ), &
    real ( 0.39638826473124717315D-01, kind = 8 ), &
    real ( 0.38450072575004151871D-01, kind = 8 ), &
    real ( 0.35273216868317898556D-01, kind = 8 ), &
    real ( 0.31768535282502272742D-01, kind = 8 ), &
    real ( 0.28894408288051391369D-01, kind = 8 ), &
    real ( 0.24463284011678541180D-01, kind = 8 ), &
    real ( 0.41053540139998941517D+00, kind = 8 ), &
    real ( 0.44993502381204990817D+00, kind = 8 ), &
    real ( 0.97220515514243332184D+00, kind = 8 ), &
    real ( 0.83764237105104371193D+02, kind = 8 ), &
    real ( 0.80327744952044756016D+05, kind = 8 ), &
    real ( 0.15514138847749108298D+06, kind = 8 ), &
    real ( 0.11995859641733262114D+07, kind = 8 ), &
    real ( 0.21472868855967642259D+08, kind = 8 ), &
    real ( 0.45564115351632913590D+09, kind = 8 ), &
    real ( 0.32980722582904761929D+12, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (  -0.0019531250D+00, kind = 8 ), &
...
!
  ...
  real ( kind = 8 ), parameter :: one = real ( 1.0, kind = 8 )
  real ( kind = 8 ) t
  real ( kind = 8 ), parameter :: twobpi = real ( 0.63661977236758134308d0, &
    kind = 8 )
  real ( kind = 8 ) x
  real ( kind = 8 ), parameter :: xlow = real ( 7.4505806d-9, kind = 8 )
  real ( kind = 8 ), parameter :: xupper = real ( 4.5036d15, kind = 8 )
  real ( kind = 8 ) xvalue
  real ( kind = 8 ), parameter :: zero = 0.0D+00
...
!
  ...
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.19531241721588483191D-02, kind = 8 ), &
    real ( -0.39062433772980711281D-02, kind = 8 ), &
    real (  0.78124470192576499535D-02, kind = 8 ), &
    real (  0.15624576181996527280D-01, kind = 8 ), &
    real ( -0.31246610349485401551D-01, kind = 8 ), &
    real (  0.62472911335014397321D-01, kind = 8 ), &
    real (  0.12478419717389654039D+00, kind = 8 ), &
    real ( -0.24830175098230686908D+00, kind = 8 ), &
    real (  0.48722235829452235711D+00, kind = 8 ), &
    real (  0.91596559417721901505D+00, kind = 8 ), &
    real (  0.12749694484943800618D+01, kind = 8 ), &
    real ( -0.15760154034463234224D+01, kind = 8 ), &
    real (  0.24258878412859089996D+01, kind = 8 ), &
    real (  0.33911633326292997361D+01, kind = 8 ), &
    real (  0.44176450919422186583D+01, kind = 8 ), &
    real ( -0.47556713749547247774D+01, kind = 8 ), &
    real (  0.50961912150934111303D+01, kind = 8 ), &
    real (  0.53759175735714876256D+01, kind = 8 ), &
    real ( -0.61649904785027487422D+01, kind = 8 ), &
    real (  0.72437843013083534973D+01, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
    real (  -0.0039062500D+00, kind = 8 ), &
    real (   0.0078125000D+00, kind = 8 ), &
    real (   0.0156250000D+00, kind = 8 ), &
    real (  -0.0312500000D+00, kind = 8 ), &
    real (   0.0625000000D+00, kind = 8 ), &
    real (   0.1250000000D+00, kind = 8 ), &
    real (  -0.2500000000D+00, kind = 8 ), &
    real (   0.5000000000D+00, kind = 8 ), &
    real (   1.0000000000D+00, kind = 8 ), &
    ...
    real (   4.0000000000D+00, kind = 8 ), &
    real (   8.0000000000D+00, kind = 8 ), &
    real (  16.0000000000D+00, kind = 8 ), &
    real ( -20.0000000000D+00, kind = 8 ), &
    real (  25.0000000000D+00, kind = 8 ), &
    real (  30.0000000000D+00, kind = 8 ), &
    real ( -50.0000000000D+00, kind = 8 ), &
    real ( 100.0000000000D+00, kind = 8 ) /)       
...
!
  ...
  real ( kind = 8 ), parameter :: ateen = real ( 18.0D+00, kind = 8 )
  real ( kind = 8 ) bessel_i0_int
  real ( kind = 8 ) cheval
  real ( kind = 8 ), parameter :: half = 0.5D+00
  integer ind
  real ( kind = 8 ), parameter :: lnr2pi = &
    real ( 0.91893853320467274178d0, kind = 8 ) 
  integer, parameter :: nterm1 = 25
  integer, parameter :: nterm2 = 27
  real ( kind = 8 ) t
  real ( kind = 8 ) temp
  real ( kind = 8 ), parameter :: thirt6 = real ( 36.0D+00, kind = 8 )
  real ( kind = 8 ), parameter :: three = 3.0D+00
  real ( kind = 8 ) x
  real ( kind = 8 ), parameter :: xhigh = real ( 713.758339d0, kind = 8 )
  real ( kind = 8 ), parameter :: xlow = real ( 0.5161914d-7, kind = 8 )
  real ( kind = 8 ) xvalue
  real ( kind = 8 ), parameter :: zero = 0.0D+00
...
!
  ...
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.19531256208818052282D-02, kind = 8 ), &
    real ( -0.39062549670565734544D-02, kind = 8 ), &
    real (  0.62520348032546565850D-01, kind = 8 ), &
    real (  0.12516285581366971819D+00, kind = 8 ), &
    real ( -0.51051480879740303760D+00, kind = 8 ), &
    real (  0.10865210970235898158D+01, kind = 8 ), &
    real (  0.27750019054282535299D+01, kind = 8 ), &
    real ( -0.13775208868039716639D+02, kind = 8 ), &
    real (  0.46424372058106108576D+03, kind = 8 ), &
    real (  0.64111867658021584522D+07, kind = 8 ), &
    real ( -0.10414860803175857953D+08, kind = 8 ), &
    real (  0.44758598913855743089D+08, kind = 8 ), &
    real ( -0.11852985311558287888D+09, kind = 8 ), &
    real (  0.31430078220715992752D+09, kind = 8 ), &
    real ( -0.83440212900794309620D+09, kind = 8 ), &
    real (  0.22175367579074298261D+10, kind = 8 ), &
    real (  0.58991731842803636487D+10, kind = 8 ), &
    real ( -0.41857073244691522147D+11, kind = 8 ), &
    real (  0.79553885818472357663D+12, kind = 8 ), &
    real (  0.15089715082719201025D+17, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
    ...
    real (  -4.0000000000D+00, kind = 8 ), &
    real (   8.0000000000D+00, kind = 8 ), &
    real (  18.0000000000D+00, kind = 8 ), &
    real ( -18.5000000000D+00, kind = 8 ), &
    real (  20.0000000000D+00, kind = 8 ), &
    real ( -21.0000000000D+00, kind = 8 ), &
    real (  22.0000000000D+00, kind = 8 ), &
    real ( -23.0000000000D+00, kind = 8 ), &
    real (  24.0000000000D+00, kind = 8 ), &
    real (  25.0000000000D+00, kind = 8 ), &
    real ( -27.0000000000D+00, kind = 8 ), &
    real (  30.0000000000D+00, kind = 8 ), &
    real (  40.0000000000D+00, kind = 8 ) /)       
...
!
  ...
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.97656242238978822427D-03, kind = 8 ), &
    real (  0.39062450329491108875D-02, kind = 8 ), &
    real ( -0.62479657927917933620D-01, kind = 8 ), &
    real (  0.12483733492120479139D+00, kind = 8 ), &
    real ( -0.48968050664604505505D+00, kind = 8 ), &
    real (  0.91973041008976023931D+00, kind = 8 ), &
    real ( -0.14257702931970265690D+01, kind = 8 ), &
    real (  0.10247341594606064818D+01, kind = 8 ), &
    real ( -0.12107468348304501655D+01, kind = 8 ), &
    real (  0.11008652032736190799D+01, kind = 8 ), &
    real ( -0.10060334829904124192D+01, kind = 8 ), &
    real (  0.81330572662485953519D+00, kind = 8 ), &
    real ( -0.10583788214211277585D+01, kind = 8 ), &
    real (  0.87101492116545875169D+00, kind = 8 ), &
    real ( -0.88424908882547488420D+00, kind = 8 ), &
    real (  0.11257761503599914603D+01, kind = 8 ), &
    real ( -0.90141212258183461184D+00, kind = 8 ), &
    real (  0.91441344369647797803D+00, kind = 8 ), &
    real ( -0.94482281938334394886D+00, kind = 8 ), &
    real (  0.92266255696016607257D+00, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0009765625D+00, kind = 8 ), &
    ...
    real (  40.0000000000D+00, kind = 8 ), &
    real ( -50.0000000000D+00, kind = 8 ), &
    real (  75.0000000000D+00, kind = 8 ), &
    real ( -80.0000000000D+00, kind = 8 ), &
    real ( 100.0000000000D+00, kind = 8 ) /)       
...
!
  ...
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.78587929563466784589D-02, kind = 8 ), &
    real ( 0.26019991617330578111D-01, kind = 8 ), &
    real ( 0.24311842237541167904D+00, kind = 8 ), &
    real ( 0.39999633750480508861D+00, kind = 8 ), &
    real ( 0.92710252093114907345D+00, kind = 8 ), &
    real ( 0.12425098486237782662D+01, kind = 8 ), &
    real ( 0.14736757343168286825D+01, kind = 8 ), &
    real ( 0.15606495706051741364D+01, kind = 8 ), &
    real ( 0.15673873907283660493D+01, kind = 8 ), &
    real ( 0.15696345532693743714D+01, kind = 8 ), &
    real ( 0.15701153443250786355D+01, kind = 8 ), &
    real ( 0.15706574852894436220D+01, kind = 8 ), &
    real ( 0.15707793116159788598D+01, kind = 8 ), &
    real ( 0.15707942066465767196D+01, kind = 8 ), &
    real ( 0.15707962315469192247D+01, kind = 8 ), &
    real ( 0.15707963262340149876D+01, kind = 8 ), &
    real ( 0.15707963267948756308D+01, kind = 8 ), &
    real ( 0.15707963267948966192D+01, kind = 8 ), &
    real ( 0.15707963267948966192D+01, kind = 8 ), &
    real ( 0.15707963267948966192D+01, kind = 8 )  /)
  integer n_data
  real ( kind = 8 ) x
...
!
  real ( kind = 8 ) bessel_y0_int
  real ( kind = 8 ) cheval
  real ( kind = 8 ), parameter :: nine = real ( 9.0, kind = 8 )
  integer, parameter :: nterm1 = 22
  integer, parameter :: nterm2 = 22
  integer, parameter :: nterm3 = 17
  integer, parameter :: nterm4 = 15
  real ( kind = 8 ), parameter :: one = real ( 1.0, kind = 8 )
  real ( kind = 8 ), parameter :: sixten = real ( 16.0, kind = 8 )
  real ( kind = 8 ) x
  real ( kind = 8 ) xvalue
  real ( kind = 8 ), parameter :: zero = 0.0D+00
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( -0.91442642860172110926D-02, kind = 8 ), &
    real ( -0.29682047390397591290D-01, kind = 8 ), &
    real ( -0.25391431276585388961D+00, kind = 8 ), &
    real ( -0.56179545591464028187D+00, kind = 8 ), &
    real ( -0.63706937660742309754D+00, kind = 8 ), &
    real ( -0.28219285008510084123D+00, kind = 8 ), &
    real (  0.38366964785312561103D+00, kind = 8 ), &
    real ( -0.12595061285798929390D+00, kind = 8 ), &
    real (  0.24129031832266684828D+00, kind = 8 ), &
    real (  0.17138069757627037938D+00, kind = 8 ), &
    real (  0.18958142627134083732D+00, kind = 8 ), &
    real (  0.17203846136449706946D+00, kind = 8 ), &
    real ( -0.16821597677215029611D+00, kind = 8 ), &
    real ( -0.93607927351428988679D-01, kind = 8 ), &
    real (  0.88229711948036648408D-01, kind = 8 ), &
    real ( -0.89324662736274161841D-02, kind = 8 ), &
    real ( -0.54814071000063488284D-01, kind = 8 ), &
    real ( -0.94958246003466381588D-01, kind = 8 ), &
    real ( -0.19598064853404969850D-01, kind = 8 ), &
    real ( -0.83084772357154773468D-02, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
    ...
    real (  10.0000000000D+00, kind = 8 ), &
    real (  16.0000000000D+00, kind = 8 ), &
    real (  16.2500000000D+00, kind = 8 ), &
    real (  17.0000000000D+00, kind = 8 ), &
    real (  20.0000000000D+00, kind = 8 ), &
    real (  25.0000000000D+00, kind = 8 ), &
    real (  30.0000000000D+00, kind = 8 ), &
    real (  40.0000000000D+00, kind = 8 ), &
    real (  50.0000000000D+00, kind = 8 ), &
    real (  70.0000000000D+00, kind = 8 ), &
    real ( 100.0000000000D+00, kind = 8 ), &
    real ( 125.0000000000D+00, kind = 8 ) /)       
!
  if ( n_data < 0 ) then
    n_data = 0
  end if
...
!
  ...
  integer i
  real ( kind = 8 ) :: t
  real ( kind = 8 ), parameter :: test = real ( 0.6, kind = 8 )
  real ( kind = 8 ) :: tt
  real ( kind = 8 ), parameter :: two = 2.0D+00
  real ( kind = 8 ) :: u0
  real ( kind = 8 ) :: u1
...
!
  ...
  real ( kind = 8 ), parameter :: one = real ( 1.0, kind = 8 )
  real ( kind = 8 ), parameter :: pi = real ( 3.1415926535897932385d0, kind = 8 )
  real ( kind = 8 ), parameter :: pisq = &
    real ( 9.8696044010893586188d0, kind = 8 )
  real ( kind = 8 ) t
  real ( kind = 8 ) x
  real ( kind = 8 ) xvalue
  real ( kind = 8 ), parameter :: zero = 0.0D+00
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.14137352886760576684D-01, kind = 8 ), &
    real (  0.13955467081981281934D+00, kind = 8 ), &
    real ( -0.38495732156574238507D+00, kind = 8 ), &
    real (  0.84831187770367927099D+00, kind = 8 ), &
    real (  0.10139591323607685043D+01, kind = 8 ), &
    real ( -0.93921859275409211003D+00, kind = 8 ), &
    real (  0.72714605086327924743D+00, kind = 8 ), &
    real (  0.43359820323553277936D+00, kind = 8 ), &
    real ( -0.98026209391301421161D-01, kind = 8 ), &
    real ( -0.56814394442986978080D+00, kind = 8 ), &
    real ( -0.70969701784448921625D+00, kind = 8 ), &
    real (  0.99282013254695671871D+00, kind = 8 ), &
    real ( -0.98127747477447367875D+00, kind = 8 ), &
    real ( -0.64078266570172320959D+00, kind = 8 ), &
    real (  0.86027963733231192456D+00, kind = 8 ), &
    real (  0.39071647608680211043D+00, kind = 8 ), &
    real (  0.47574793926539191502D+00, kind = 8 ), &
    real (  0.10105014481412878253D+01, kind = 8 ), &
    real (  0.96332089044363075154D+00, kind = 8 ), &
    real ( -0.61782699481929311757D+00, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
    ...
    real (  -1.5000000000D+00, kind = 8 ), &
    real (   2.0000000000D+00, kind = 8 ), &
    real (   2.5000000000D+00, kind = 8 ), &
    real (  -3.0000000000D+00, kind = 8 ), &
    real (   4.0000000000D+00, kind = 8 ), &
    real (   4.2500000000D+00, kind = 8 ), &
    real (  -5.0000000000D+00, kind = 8 ), &
    real (   5.5000000000D+00, kind = 8 ), &
    real (   6.0000000000D+00, kind = 8 ), &
    real (   8.0000000000D+00, kind = 8 ), &
    real ( -10.0000000000D+00, kind = 8 ), &
    real (  15.0000000000D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.99951182471380889183D+00, kind = 8 ), &
    real (  0.99221462647120597836D+00, kind = 8 ), &
    real (  0.96918395997895308324D+00, kind = 8 ), &
    real (  0.88192715679060552968D+00, kind = 8 ), &
    real (  0.77750463411224827642D+00, kind = 8 ), &
    real (  0.68614531078940204342D+00, kind = 8 ), &
    real (  0.60694728460981007205D+00, kind = 8 ), &
    real (  0.53878956907785587703D+00, kind = 8 ), &
    real (  0.48043521957304283829D+00, kind = 8 ), &
    real (  0.38814802129793784501D+00, kind = 8 ), &
    real (  0.36930802829242526815D+00, kind = 8 ), &
    real (  0.32087619770014612104D+00, kind = 8 ), &
    real (  0.29423996623154246701D+00, kind = 8 ), &
    real (  0.27126046678502189985D+00, kind = 8 ), &
    real (  0.20523930310221503723D+00, kind = 8 ), &
    real (  0.16444346567994602563D+00, kind = 8 ), &
    real (  0.10966194482735821276D+00, kind = 8 ), &
    real (  0.82246701178200016086D-01, kind = 8 ), &
    real (  0.54831135561510852445D-01, kind = 8 ), &
    real (  0.32898681336964528729D-01, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.99934911727904599738D+00, kind = 8 ), &
    real (  0.98962402299599181205D+00, kind = 8 ), &
    real (  0.95898426200345986743D+00, kind = 8 ), &
    real (  0.84372119334725358934D+00, kind = 8 ), &
    real (  0.70787847562782928288D+00, kind = 8 ), &
    real (  0.59149637225671282917D+00, kind = 8 ), &
    real (  0.49308264399053185014D+00, kind = 8 ), &
    real (  0.41079413579749669069D+00, kind = 8 ), &
    real (  0.34261396060786351671D+00, kind = 8 ), &
    real (  0.24055368752127897660D+00, kind = 8 ), &
    real (  0.22082770061202308232D+00, kind = 8 ), &
    real (  0.17232915939014138975D+00, kind = 8 ), &
    real (  0.14724346738730182894D+00, kind = 8 ), &
    real (  0.12666919046715789982D+00, kind = 8 ), &
    real (  0.74268805954862854626D-01, kind = 8 ), &
    real (  0.47971498020121871622D-01, kind = 8 ), &
    real (  0.21369201683658373846D-01, kind = 8 ), &
    real (  0.12020564476446432799D-01, kind = 8 ), &
    real (  0.53424751249537071952D-02, kind = 8 ), &
    real (  0.19232910450553508562D-02, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.99926776885985461940D+00, kind = 8 ), &
    real (  0.98833007755734698212D+00, kind = 8 ), &
    real (  0.95390610472023510237D+00, kind = 8 ), &
    real (  0.82496296897623372315D+00, kind = 8 ), &
    real (  0.67441556407781468010D+00, kind = 8 ), &
    real (  0.54710665141286285468D+00, kind = 8 ), &
    real (  0.44112847372762418113D+00, kind = 8 ), &
    real (  0.35413603481042394211D+00, kind = 8 ), &
    real (  0.28357982814342246206D+00, kind = 8 ), &
    real (  0.18173691382177474795D+00, kind = 8 ), &
    real (  0.16277924385112436877D+00, kind = 8 ), &
    real (  0.11759741179993396450D+00, kind = 8 ), &
    real (  0.95240802723158889887D-01, kind = 8 ), &
    real (  0.77581324733763020269D-01, kind = 8 ), &
    real (  0.36560295673194845002D-01, kind = 8 ), &
    real (  0.19295765690345489563D-01, kind = 8 ), &
    real (  0.57712632276188798621D-02, kind = 8 ), &
    real (  0.24352200674805479827D-02, kind = 8 ), &
    real (  0.72154882216335666096D-03, kind = 8 ), &
    real (  0.15585454565440389896D-03, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.99921896192761576256D+00, kind = 8 ), &
    real (  0.98755425280996071022D+00, kind = 8 ), &
    real (  0.95086788606389739976D+00, kind = 8 ), &
    real (  0.81384569172034042516D+00, kind = 8 ), &
    real (  0.65487406888673697092D+00, kind = 8 ), &
    real (  0.52162830964878715188D+00, kind = 8 ), &
    real (  0.41189273671788528876D+00, kind = 8 ), &
    real (  0.32295434858707304628D+00, kind = 8 ), &
    real (  0.25187863642883314410D+00, kind = 8 ), &
    real (  0.15185461258672022043D+00, kind = 8 ), &
    real (  0.13372661145921413299D+00, kind = 8 ), &
    real (  0.91471377664481164749D-01, kind = 8 ), &
    real (  0.71227828197462523663D-01, kind = 8 ), &
    real (  0.55676547822738862783D-01, kind = 8 ), &
    real (  0.21967566525574960096D-01, kind = 8 ), &
    real (  0.96736755602711590082D-02, kind = 8 ), &
    real (  0.19646978158351837850D-02, kind = 8 ), &
    real (  0.62214648623965450200D-03, kind = 8 ), &
    real (  0.12289514092077854510D-03, kind = 8 ), &
    real (  0.15927210319002161231D-04, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.19531249963620212007D-02, kind = 8 ), &
    real (  0.78124990686775522671D-02, kind = 8 ), &
    real (  0.31249761583499728667D-01, kind = 8 ), &
    real (  0.12493899888803079984D+00, kind = 8 ), &
    real (  0.48491714311363971332D+00, kind = 8 ), &
    real (  0.80751118213967145286D+00, kind = 8 ), &
    real (  0.86889265412623270696D+00, kind = 8 ), &
    real (  0.88861722235357162648D+00, kind = 8 ), &
    real (  0.89286018500218176869D+00, kind = 8 ), &
    real (  0.89295351429387631138D+00, kind = 8 ), &
    real (  0.89297479112737843939D+00, kind = 8 ), &
    real (  0.89297880579798112220D+00, kind = 8 ), &
    real (  0.89297950317496621294D+00, kind = 8 ), &
    real (  0.89297951152951902903D+00, kind = 8 ), &
    real (  0.89297951156918122102D+00, kind = 8 ), &
    real (  0.89297951156924734716D+00, kind = 8 ), &
    real (  0.89297951156924917298D+00, kind = 8 ), &
    real (  0.89297951156924921121D+00, kind = 8 ), &
    real (  0.89297951156924921122D+00, kind = 8 ), &
    real (  0.89297951156924921122D+00, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    ...
    real (  2.0000000000D+00, kind = 8 ), &
    real (  2.1250000000D+00, kind = 8 ), &
    real (  2.2500000000D+00, kind = 8 ), &
    real (  2.5000000000D+00, kind = 8 ), &
    real (  2.7500000000D+00, kind = 8 ), &
    real (  3.0000000000D+00, kind = 8 ), &
    real (  3.1250000000D+00, kind = 8 ), &
    real (  3.2500000000D+00, kind = 8 ), &
    real (  3.5000000000D+00, kind = 8 ), &
    real (  3.7500000000D+00, kind = 8 ), &
    real (  4.0000000000D+00, kind = 8 ) /)       
!
  if ( n_data < 0 ) then
    n_data = 0
...
!
  ...
  integer, parameter :: nterm2 = 20
  real ( kind = 8 ), parameter :: one = real ( 1.0, kind = 8 )
  real ( kind = 8 ), parameter :: six = real ( 6.0, kind = 8 )
  real ( kind = 8 ), parameter :: two = 2.0D+00
  real ( kind = 8 ) x
  real ( kind = 8 ) xvalue
  real ( kind = 8 ), parameter :: zero = 0.0D+00
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.59531540040441651584D+01, kind = 8 ), &
    real (  0.45769601268624494109D+01, kind = 8 ), &
    real (  0.32288921331902217638D+01, kind = 8 ), &
    real (  0.19746110873568719362D+01, kind = 8 ), &
    real (  0.96356046208697728563D+00, kind = 8 ), &
    real (  0.60513365250334458174D+00, kind = 8 ), &
    real (  0.51305506459532198016D+00, kind = 8 ), &
    real (  0.44598602820946133091D+00, kind = 8 ), &
    real (  0.37344458206879749357D+00, kind = 8 ), &
    real (  0.35433592884953063055D+00, kind = 8 ), &
    real (  0.33712156518881920994D+00, kind = 8 ), &
    real (  0.29436170729362979176D+00, kind = 8 ), &
    real (  0.25193499644897222840D+00, kind = 8 ), &
    real (  0.22028778222123939276D+00, kind = 8 ), &
    real (  0.19575258237698917033D+00, kind = 8 ), &
    real (  0.17616303166670699424D+00, kind = 8 ), &
    real (  0.16015469479664778673D+00, kind = 8 ), &
    real (  0.14096116876193391066D+00, kind = 8 ), &
    real (  0.13554987191049066274D+00, kind = 8 ), &
    real (  0.11751605060085098084D+00, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (  0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.99875755515461749793D+00, kind = 8 ), &
    real ( 0.99011358230706643807D+00, kind = 8 ), &
    real ( 0.92419435310023947018D+00, kind = 8 ), &
    real ( 0.73624267134714273902D+00, kind = 8 ), &
    real ( 0.55582269181411744686D+00, kind = 8 ), &
    real ( 0.34215154434462160628D+00, kind = 8 ), &
    real ( 0.17087174888774706539D+00, kind = 8 ), &
    real ( 0.81081008709219208918D-01, kind = 8 ), &
    real ( 0.53449421441089580702D-01, kind = 8 ), &
    real ( 0.39950321008923244846D-01, kind = 8 ), &
    real ( 0.39330637437584921392D-01, kind = 8 ), &
    real ( 0.37582274342808670750D-01, kind = 8 ), &
    real ( 0.31912486554480390343D-01, kind = 8 ), &
    real ( 0.25506146883504738403D-01, kind = 8 ), &
    real ( 0.21244480317825292412D-01, kind = 8 ), &
    real ( 0.15925498348551684335D-01, kind = 8 ), &
    real ( 0.12737506927242585015D-01, kind = 8 ), &
    real ( 0.84897750814784916847D-02, kind = 8 ), &
    real ( 0.63668349178454469153D-02, kind = 8 ), &
    real ( 0.50932843163122551114D-02, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.97575346155386267134D-03, kind = 8 ), &
    real ( 0.77609293280609272733D-02, kind = 8 ), &
    real ( 0.59302966404545373770D-01, kind = 8 ), &
    real ( 0.20395212276737365307D+00, kind = 8 ), &
    real ( 0.33839472293667639038D+00, kind = 8 ), &
    real ( 0.48787706726961324579D+00, kind = 8 ), &
    real ( 0.59018734196576517506D+00, kind = 8 ), &
    real ( 0.62604539530312149476D+00, kind = 8 ), &
    real ( 0.63209315274909764698D+00, kind = 8 ), &
    real ( 0.63410179313235359215D+00, kind = 8 ), &
    real ( 0.63417966797578128188D+00, kind = 8 ), &
    real ( 0.63439268632392089434D+00, kind = 8 ), &
    real ( 0.63501579073257770690D+00, kind = 8 ), &
    real ( 0.63559616677359459337D+00, kind = 8 ), &
    real ( 0.63591001826697110312D+00, kind = 8 ), &
    real ( 0.63622113181751073643D+00, kind = 8 ), &
    real ( 0.63636481702133606597D+00, kind = 8 ), &
    real ( 0.63650653499619902120D+00, kind = 8 ), &
    real ( 0.63655609126300261851D+00, kind = 8 ), &
    real ( 0.63657902087183929223D+00, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.12417639065161393857D-08, kind = 8 ), &
    real ( 0.79473344770001088225D-07, kind = 8 ), &
    real ( 0.50867598186208834198D-05, kind = 8 ), &
    real ( 0.32603097901207200319D-03, kind = 8 ), &
    real ( 0.21380536815408214419D-01, kind = 8 ), &
    real ( 0.18753816902083824050D+00, kind = 8 ), &
    real ( 0.83051199971883645115D+00, kind = 8 ), &
    real ( 0.18854362426679034904D+01, kind = 8 ), &
    real ( 0.21315988986516411053D+01, kind = 8 ), &
    real ( 0.21771120185613427221D+01, kind = 8 ), &
    real ( 0.22921027921896650849D+01, kind = 8 ), &
    real ( 0.39137195028784495586D+01, kind = 8 ), &
    real ( 0.43513563983836427904D+01, kind = 8 ), &
    real ( 0.44200644968478185898D+01, kind = 8 ), &
    real ( 0.65656013133623829156D+01, kind = 8 ), &
    real ( 0.10825504661504599479D+02, kind = 8 ), &
    real ( 0.13365512855474227325D+02, kind = 8 ), &
    real ( 0.21131002685639959927D+02, kind = 8 ), &
    real ( 0.34838236589449117389D+02, kind = 8 ), &
    real ( 0.69657062437837394278D+02, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.21901065985698662316D-15, kind = 8 ), &
    real ( 0.22481399438625244761D-12, kind = 8 ), &
    real ( 0.23245019579558857124D-09, kind = 8 ), &
    real ( 0.24719561475975007037D-06, kind = 8 ), &
    real ( 0.28992610989833245669D-03, kind = 8 ), &
    real ( 0.10698146390809715091D-01, kind = 8 ), &
    real ( 0.89707650964424730705D-01, kind = 8 ), &
    real ( 0.40049605719592888440D+00, kind = 8 ), &
    real ( 0.30504104398079096598D+01, kind = 8 ), &
    real ( 0.11367704858439426431D+02, kind = 8 ), &
    real ( 0.12960679405324786954D+02, kind = 8 ), &
    real ( 0.18548713944748505675D+02, kind = 8 ), &
    real ( 0.27866273821903121400D+02, kind = 8 ), &
    real ( 0.51963334071699323351D+02, kind = 8 ), &
    real ( 0.10861016747891228129D+03, kind = 8 ), &
    real ( 0.15378903316556621624D+03, kind = 8 ), &
    real ( 0.19302665532558721516D+03, kind = 8 ), &
    real ( 0.19636850166006541482D+03, kind = 8 ), &
    real ( 0.19651946766008214217D+03, kind = 8 ), &
    real ( 0.19651956920868316152D+03, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.12433974658847434366d-02, kind = 8 ), &
    real ( -0.49735582423748415045d-02, kind = 8 ), &
    real (  0.39771469054536941564d-01, kind = 8 ), &
    real ( -0.15805246001653314198d+00, kind = 8 ), &
    real (  0.56865662704828795099d+00, kind = 8 ), &
    real (  0.66598399314899916605d+00, kind = 8 ), &
    real (  0.79085884950809589255d+00, kind = 8 ), &
    real ( -0.13501457342248639716d+00, kind = 8 ), &
    real (  0.20086479668164503137d+00, kind = 8 ), &
    real ( -0.11142097800261991552d+00, kind = 8 ), &
    real ( -0.17026804865989885869d+00, kind = 8 ), &
    real ( -0.13544931808186467594d+00, kind = 8 ), &
    real (  0.94393698081323450897d-01, kind = 8 ), &
    real ( -0.10182482016001510271d+00, kind = 8 ), &
    real (  0.96098421554162110012d-01, kind = 8 ), &
    real ( -0.85337674826118998952d-01, kind = 8 ), &
    real ( -0.76882290637052720045d-01, kind = 8 ), &
    real (  0.47663833591418256339d-01, kind = 8 ), &
    real ( -0.70878751689647343204d-01, kind = 8 ), &
    real (  0.65752908073352785368d-01, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (    0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.80950369576367526071D-06, kind = 8 ), &
    real ( 0.12952009724113229165D-04, kind = 8 ), &
    real ( 0.82871615165407083021D-03, kind = 8 ), &
    real ( 0.13207748375849572564D-01, kind = 8 ), &
    real ( 0.19845733620194439894D+00, kind = 8 ), &
    real ( 0.29853823231804706294D+00, kind = 8 ), &
    real ( 0.64676372828356211712D+00, kind = 8 ), &
    real ( 0.10697266613089193593D+01, kind = 8 ), &
    real ( 0.38831308000420560970D+00, kind = 8 ), &
    real ( 0.74854243745107710333D+00, kind = 8 ), &
    real ( 0.84664854642567359993D+00, kind = 8 ), &
    real ( 0.58385732464244384564D+00, kind = 8 ), &
    real ( 0.80600584524215772824D+00, kind = 8 ), &
    real ( 0.53880362132692947616D+00, kind = 8 ), &
    real ( 0.72175037834698998506D+00, kind = 8 ), &
    real ( 0.58007844794544189900D+00, kind = 8 ), &
    real ( 0.60151910385440804463D+00, kind = 8 ), &
    real ( 0.70611511147286827018D+00, kind = 8 ), &
    real ( 0.61631110327201338454D+00, kind = 8 ), &
    real ( 0.62778480765443656489D+00, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (    0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (  0.12433985199262820188D-02, kind = 8 ), &
    real ( -0.19896526647882937004D-01, kind = 8 ), &
    real (  0.79715713253115014945D-01, kind = 8 ), &
    real ( -0.32724069939418078025D+00, kind = 8 ), &
    real (  0.71024318593789088874D+00, kind = 8 ), &
    real (  0.19374337579914456612D+01, kind = 8 ), &
    real ( -0.11131050203248583431D+02, kind = 8 ), &
    real (  0.16850062034703267148D+03, kind = 8 ), &
    real ( -0.28156522493745948555D+04, kind = 8 ), &
    real (  0.89344618796978400815D+06, kind = 8 ), &
    real (  0.11382025002851451057D+07, kind = 8 ), &
    real ( -0.23549701855860190304D+07, kind = 8 ), &
    real (  0.43558282527641046718D+08, kind = 8 ), &
    real (  0.49993516476037957165D+09, kind = 8 ), &
    real ( -0.57745606064408041689D+10, kind = 8 ), &
    real (  0.78167229782395624524D+12, kind = 8 ), &
    real ( -0.14894774793419899908D+17, kind = 8 ), &
    real (  0.29325537838493363267D+21, kind = 8 ), &
    real (  0.58940770556098011683D+25, kind = 8 ), &
    real ( -0.12015889579125463605D+30, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
    ...
    real ( -17.0000000000D+00, kind = 8 ), &
    real (  20.0000000000D+00, kind = 8 ), &
    real (  22.5000000000D+00, kind = 8 ), &
    real ( -25.0000000000D+00, kind = 8 ), &
    real (  30.0000000000D+00, kind = 8 ), &
    real ( -40.0000000000D+00, kind = 8 ), &
    real (  50.0000000000D+00, kind = 8 ), &
    real (  60.0000000000D+00, kind = 8 ), &
    real ( -70.0000000000D+00, kind = 8 ) /) 
!
  if ( n_data < 0 ) then
    n_data = 0
...
!
  real ( kind = 8 ) cheval
  real ( kind = 8 ), parameter :: four = real ( 4.0, kind = 8 )
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.80950410749865126939d-06, kind = 8 ), &
    real ( 0.20724649092571514607d-03, kind = 8 ), &
    real ( 0.33191834066894516744d-02, kind = 8 ), &
    real ( 0.53942182623522663292d-01, kind = 8 ), &
    real ( 0.22676438105580863683d+00, kind = 8 ), &
    real ( 0.11027597873677158176d+01, kind = 8 ), &
    real ( 0.91692778117386847344d+01, kind = 8 ), &
    real ( 0.15541656652426660966d+03, kind = 8 ), &
    real ( 0.26703582852084829694d+04, kind = 8 ), &
    real ( 0.86505880175304633906d+06, kind = 8 ), &
    real ( 0.11026046613094942620d+07, kind = 8 ), &
    real ( 0.22846209494153934787d+07, kind = 8 ), &
    real ( 0.42454972750111979449d+08, kind = 8 ), &
    real ( 0.48869614587997695539d+09, kind = 8 ), &
    real ( 0.56578651292431051863d+10, kind = 8 ), &
    real ( 0.76853203893832108948d+12, kind = 8 ), &
    real ( 0.14707396163259352103d+17, kind = 8 ), &
    real ( 0.29030785901035567967d+21, kind = 8 ), &
    real ( 0.58447515883904682813d+25, kind = 8 ), &
    real ( 0.11929750788892311875d+30, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) cheval
  real ( kind = 8 ), parameter :: eight = real ( 8.0D+00, kind = 8 )
  real ( kind = 8 ), parameter :: four = real ( 4.0D+00, kind = 8 )
  real ( kind = 8 ), parameter :: half = real ( 0.5D+00, kind = 8 )
  integer, parameter :: nterm1 = 12
  integer, parameter :: nterm2 = 10
  integer, parameter :: nterm3 = 21
  real ( kind = 8 ), parameter :: one = real ( 1.0D+00, kind = 8 )
  real ( kind = 8 ) synch1
  real ( kind = 8 ), parameter :: three = real ( 3.0D+00, kind = 8 )
  real ( kind = 8 ) x
  real ( kind = 8 ) xvalue
  real ( kind = 8 ), parameter :: zero = real ( 0.0D+00, kind = 8 )
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real (   0.26514864547487397044D+00, kind = 8 ), &
    real (   0.62050129979079045645D+00, kind = 8 ), &
    real (   0.85112572132368011206D+00, kind = 8 ), &
    real (   0.87081914687546885094D+00, kind = 8 ), &
    real (   0.65142281535536396975D+00, kind = 8 ), &
    real (   0.45064040920322354579D+00, kind = 8 ), &
    real (   0.30163590285073940285D+00, kind = 8 ), &
    real (   0.19814490804441305867D+00, kind = 8 ), &
    real (   0.12856571000906381300D+00, kind = 8 ), &
    real (   0.52827396697866818297D-01, kind = 8 ), &
    real (   0.42139298471720305542D-01, kind = 8 ), &
    real (   0.21248129774981984268D-01, kind = 8 ), &
    real (   0.13400258907505536491D-01, kind = 8 ), &
    real (   0.84260797314108699935D-02, kind = 8 ), &
    real (   0.12884516186754671469D-02, kind = 8 ), &
    real (   0.19223826430086897418D-03, kind = 8 ), &
    real (   0.28221070834007689394D-04, kind = 8 ), &
    real (   0.15548757973038189372D-05, kind = 8 ), &
    real (   0.11968634456097453636D-07, kind = 8 ), &
    real (   0.89564246772237127742D-10, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  ...
  real ( kind = 8 ), parameter :: one = real ( 1.0, kind = 8 )
  real ( kind = 8 ) synch2
  real ( kind = 8 ), parameter :: three = real ( 3.0, kind = 8 )
  real ( kind = 8 ), parameter :: two = 2.0D+00
  real ( kind = 8 ) x
  real ( kind = 8 ) xvalue
  real ( kind = 8 ), parameter :: zero = 0.0D+00
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.13430727275667378338D+00, kind = 8 ), &
    real ( 0.33485265272424176976D+00, kind = 8 ), &
    real ( 0.50404224110911078651D+00, kind = 8 ), &
    real ( 0.60296523236016785113D+00, kind = 8 ), &
    real ( 0.49447506210420826699D+00, kind = 8 ), &
    real ( 0.36036067860473360389D+00, kind = 8 ), &
    real ( 0.24967785497625662113D+00, kind = 8 ), &
    real ( 0.16813830542905833533D+00, kind = 8 ), &
    real ( 0.11117122348556549832D+00, kind = 8 ), &
    real ( 0.46923205826101330711D-01, kind = 8 ), &
    real ( 0.37624545861980001482D-01, kind = 8 ), &
    real ( 0.19222123172484106436D-01, kind = 8 ), &
    real ( 0.12209535343654701398D-01, kind = 8 ), &
    real ( 0.77249644268525771866D-02, kind = 8 ), &
    real ( 0.12029044213679269639D-02, kind = 8 ), &
    real ( 0.18161187569530204281D-03, kind = 8 ), &
    real ( 0.26884338006629353506D-04, kind = 8 ), &
    real ( 0.14942212731345828759D-05, kind = 8 ), &
    real ( 0.11607696854385161390D-07, kind = 8 ), &
    real ( 0.87362343746221526073D-10, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.19531247930394515480D-02, kind = 8 ), &
    real ( 0.31249152314331109004D-01, kind = 8 ), &
    real ( 0.12494577194783451032D+00, kind = 8 ), &
    real ( 0.49655363615640595865D+00, kind = 8 ), &
    real ( 0.97303256135517012845D+00, kind = 8 ), &
    real ( 0.14121978695932525805D+01, kind = 8 ), &
    real ( 0.18017185674405776809D+01, kind = 8 ), &
    real ( 0.21350385339277043015D+01, kind = 8 ), &
    real ( 0.24110500490169534620D+01, kind = 8 ), &
    real ( 0.28066664045631179931D+01, kind = 8 ), &
    real ( 0.28777421863296234131D+01, kind = 8 ), &
    real ( 0.30391706043438554330D+01, kind = 8 ), &
    real ( 0.31125074928667355940D+01, kind = 8 ), &
    real ( 0.31656687817738577185D+01, kind = 8 ), &
    real ( 0.32623520367816009184D+01, kind = 8 ), &
    real ( 0.32843291144979517358D+01, kind = 8 ), &
    real ( 0.32897895167775788137D+01, kind = 8 ), &
    real ( 0.32898672226665499687D+01, kind = 8 ), &
    real ( 0.32898681336064325400D+01, kind = 8 ), &
    real ( 0.32898681336964528724D+01, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.19073483296476379584D-05, kind = 8 ), &
    real ( 0.48826138243180786081D-03, kind = 8 ), &
    real ( 0.78074163848431205820D-02, kind = 8 ), &
    real ( 0.12370868718812031049D+00, kind = 8 ), &
    real ( 0.47984100657241749994D+00, kind = 8 ), &
    real ( 0.10269431622039754738D+01, kind = 8 ), &
    real ( 0.17063547219458658863D+01, kind = 8 ), &
    real ( 0.24539217444475937661D+01, kind = 8 ), &
    real ( 0.32106046629422467723D+01, kind = 8 ), &
    real ( 0.45792174372291563703D+01, kind = 8 ), &
    real ( 0.48722022832940370805D+01, kind = 8 ), &
    real ( 0.56143866138422732286D+01, kind = 8 ), &
    real ( 0.59984455864575470009D+01, kind = 8 ), &
    real ( 0.63033953673480961120D+01, kind = 8 ), &
    real ( 0.69579908688361166266D+01, kind = 8 ), &
    real ( 0.71503227120085929750D+01, kind = 8 ), &
    real ( 0.72110731475871876393D+01, kind = 8 ), &
    real ( 0.72123221966388461839D+01, kind = 8 ), &
    real ( 0.72123414161609465119D+01, kind = 8 ), &
    real ( 0.72123414189575656868D+01, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.24835263919461834041D-08, kind = 8 ), &
    real ( 0.10172029353616724881D-04, kind = 8 ), &
    real ( 0.65053332405940765479D-03, kind = 8 ), &
    real ( 0.41150448004155727767D-01, kind = 8 ), &
    real ( 0.31724404523442648241D+00, kind = 8 ), &
    real ( 0.10079442901142373591D+01, kind = 8 ), &
    real ( 0.22010881024333408363D+01, kind = 8 ), &
    real ( 0.38846508619156545210D+01, kind = 8 ), &
    real ( 0.59648223973714765245D+01, kind = 8 ), &
    real ( 0.10731932392998622219D+02, kind = 8 ), &
    real ( 0.11940028876819364777D+02, kind = 8 ), &
    real ( 0.15359784316882182982D+02, kind = 8 ), &
    real ( 0.17372587633093742893D+02, kind = 8 ), &
    real ( 0.19122976016053166969D+02, kind = 8 ), &
    real ( 0.23583979156921941515D+02, kind = 8 ), &
    real ( 0.25273667677030441733D+02, kind = 8 ), &
    real ( 0.25955198214572256372D+02, kind = 8 ), &
    real ( 0.25975350935212241910D+02, kind = 8 ), &
    real ( 0.25975757522084093747D+02, kind = 8 ), &
    real ( 0.25975757609067315288D+02, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.36379780361036116971D-11, kind = 8 ), &
    real ( 0.23840564453948442379D-06, kind = 8 ), &
    real ( 0.60982205372226969189D-04, kind = 8 ), &
    real ( 0.15410004586376649337D-01, kind = 8 ), &
    real ( 0.23661587923909478926D+00, kind = 8 ), &
    real ( 0.11198756851307629651D+01, kind = 8 ), &
    real ( 0.32292901663684049171D+01, kind = 8 ), &
    real ( 0.70362973105160654056D+01, kind = 8 ), &
    real ( 0.12770557691044159511D+02, kind = 8 ), &
    real ( 0.29488339015245845447D+02, kind = 8 ), &
    real ( 0.34471340540362254586D+02, kind = 8 ), &
    real ( 0.50263092218175187785D+02, kind = 8 ), &
    real ( 0.60819909101127165207D+02, kind = 8 ), &
    real ( 0.70873334429213460498D+02, kind = 8 ), &
    real ( 0.10147781242977788097D+03, kind = 8 ), &
    real ( 0.11638074540242071077D+03, kind = 8 ), &
    real ( 0.12409623901262967878D+03, kind = 8 ), &
    real ( 0.12442270155632550228D+03, kind = 8 ), &
    real ( 0.12443132790838589548D+03, kind = 8 ), &
    real ( 0.12443133061720432435D+03, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.56843405953641209574D-14, kind = 8 ), &
    real ( 0.59601180165247401484D-08, kind = 8 ), &
    real ( 0.60978424397580572815D-05, kind = 8 ), &
    real ( 0.61578909866319494394D-02, kind = 8 ), &
    real ( 0.18854360275680840514D+00, kind = 8 ), &
    real ( 0.13319251347921659134D+01, kind = 8 ), &
    real ( 0.50857202271697616755D+01, kind = 8 ), &
    real ( 0.13729222365466557122D+02, kind = 8 ), &
    real ( 0.29579592481641441292D+02, kind = 8 ), &
    real ( 0.88600835706899853768D+02, kind = 8 ), &
    real ( 0.10916037113373004909D+03, kind = 8 ), &
    real ( 0.18224323749575359518D+03, kind = 8 ), &
    real ( 0.23765383125586756031D+03, kind = 8 ), &
    real ( 0.29543246745959381136D+03, kind = 8 ), &
    real ( 0.50681244381280455592D+03, kind = 8 ), &
    real ( 0.63878231134946125623D+03, kind = 8 ), &
    real ( 0.72699203556994876111D+03, kind = 8 ), &
    real ( 0.73230331643146851717D+03, kind = 8 ), &
    real ( 0.73248692015882096369D+03, kind = 8 ), &
    real ( 0.73248700462879996604D+03, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.92518563327283409427D-17, kind = 8 ), &
    real ( 0.15521095556949867541D-09, kind = 8 ), &
    real ( 0.63516238373841716290D-06, kind = 8 ), &
    real ( 0.25638801246626135714D-02, kind = 8 ), &
    real ( 0.15665328993811649746D+00, kind = 8 ), &
    real ( 0.16538225039181097423D+01, kind = 8 ), &
    real ( 0.83763085709508211054D+01, kind = 8 ), &
    real ( 0.28078570717830763747D+02, kind = 8 ), &
    real ( 0.72009676046751991365D+02, kind = 8 ), &
    real ( 0.28174905701691911450D+03, kind = 8 ), &
    real ( 0.36660227975327792529D+03, kind = 8 ), &
    real ( 0.70556067982603601123D+03, kind = 8 ), &
    real ( 0.99661927562755629434D+03, kind = 8 ), &
    real ( 0.13288914430417403901D+04, kind = 8 ), &
    real ( 0.27987640273169129925D+04, kind = 8 ), &
    real ( 0.39721376409416504325D+04, kind = 8 ), &
    real ( 0.49913492839319899726D+04, kind = 8 ), &
    real ( 0.50781562639825019000D+04, kind = 8 ), &
    real ( 0.50820777202028708434D+04, kind = 8 ), &
    real ( 0.50820803580047164618D+04, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.15488598634539359463D-19, kind = 8 ), &
    real ( 0.41574269117845953797D-11, kind = 8 ), &
    real ( 0.68050651245227411689D-07, kind = 8 ), &
    real ( 0.10981703519563009836D-02, kind = 8 ), &
    real ( 0.13396432776187883834D+00, kind = 8 ), &
    real ( 0.21153387806998617182D+01, kind = 8 ), &
    real ( 0.14227877028750735641D+02, kind = 8 ), &
    real ( 0.59312061431647843226D+02, kind = 8 ), &
    real ( 0.18139614577043147745D+03, kind = 8 ), &
    real ( 0.93148001928992220863D+03, kind = 8 ), &
    real ( 0.12817928112604611804D+04, kind = 8 ), &
    real ( 0.28572838386329242218D+04, kind = 8 ), &
    real ( 0.43872971687877730010D+04, kind = 8 ), &
    real ( 0.62993229139406657611D+04, kind = 8 ), &
    real ( 0.16589426277154888511D+05, kind = 8 ), &
    real ( 0.27064780798797398935D+05, kind = 8 ), &
    real ( 0.38974556062543661284D+05, kind = 8 ), &
    real ( 0.40400240716905025786D+05, kind = 8 ), &
    real ( 0.40484316504120655568D+05, kind = 8 ), &
    real ( 0.40484399001892184901D+05, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
...
!
  ...
  real ( kind = 8 ), parameter :: one = real ( 1.0, kind = 8 )
  real ( kind = 8 ) rk
  real ( kind = 8 ), parameter :: rnumjn = 9.0d0
  real ( kind = 8 ) sumexp
  real ( kind = 8 ) sum2
  real ( kind = 8 ) t
  real ( kind = 8 ) tran09
...
!
  real ( kind = 8 ) fx
  real ( kind = 8 ), save, dimension ( n_max ) :: fx_vec = (/ &
    real ( 0.26469772870084897671D-22, kind = 8 ), &
    real ( 0.11367943653594246210D-12, kind = 8 ), &
    real ( 0.74428246255329800255D-08, kind = 8 ), &
    real ( 0.48022728485415366194D-03, kind = 8 ), &
    real ( 0.11700243014358676725D+00, kind = 8 ), &
    real ( 0.27648973910899914391D+01, kind = 8 ), &
    real ( 0.24716631405829192997D+02, kind = 8 ), &
    real ( 0.12827119828849828583D+03, kind = 8 ), &
    real ( 0.46842894800662208986D+03, kind = 8 ), &
    real ( 0.31673967371627895718D+04, kind = 8 ), &
    real ( 0.46140886546630195390D+04, kind = 8 ), &
    real ( 0.11952718545392302185D+05, kind = 8 ), &
    real ( 0.20001612666477027728D+05, kind = 8 ), &
    real ( 0.31011073271851366554D+05, kind = 8 ), &
    real ( 0.10352949905541130133D+06, kind = 8 ), &
    real ( 0.19743173017140591390D+06, kind = 8 ), &
    real ( 0.33826030414658460679D+06, kind = 8 ), &
    real ( 0.36179607036750755227D+06, kind = 8 ), &
    real ( 0.36360622124777561525D+06, kind = 8 ), &
    real ( 0.36360880558827162725D+06, kind = 8 ) /)
  integer n_data
  real ( kind = 8 ) x
  real ( kind = 8 ), save, dimension ( n_max ) :: x_vec = (/ &
    real (   0.0019531250D+00, kind = 8 ), &
