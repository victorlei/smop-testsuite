function [modlist,funlist,fun_info]=asec_make(typestrlist)
...
for i=1:length(typestrlist)
 ...
 if any(strcmp(typestr,com))
  kernel=[kernel,'        out=-i*log((1.0+i*sqrt(in_1**2.0-1.0))/in_1)',r];
 else
  kernel=[kernel,'        out=real(-i*log((1.0+i*sqrt(in_1**2.0-1.0))/in_1))',r];
  if any(strcmp(typestr,arr))
