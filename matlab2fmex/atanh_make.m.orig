function [modlist,funlist,fun_info]=atanh_make(typestrlist)
...
for i=1:length(typestrlist)
 ...
 if any(strcmp(typestr,com))
  kernel=[kernel,'        out=log((1.0+in_1)/(1.0-in_1))/2.0',r];
 else
  kernel=[kernel,'        out=real(log((1.0+in_1)/(1.0-in_1))/2.0)',r];
  if any(strcmp(typestr,arr))
