function [modlist,funlist,fun_info]=asin_make(typestrlist)
...
for i=1:length(typestrlist)
 ...
 if any(strcmp(typestr,com))
  kernel=[kernel,'        out=-i*log(sqrt(1.0-in_1**2)+i*in_1)',r];
 else
  kernel=[kernel,'        out=real(-i*log(sqrt(1.0-in_1**2)+i*in_1))',r];
  if any(strcmp(typestr,arr))
   kernel=[kernel,'        if (any(abs(in_1)**2.0>1.0)) then',r];
  else
   kernel=[kernel,'        if ((abs(in_1)**2.0>1.0)) then',r];
  end
