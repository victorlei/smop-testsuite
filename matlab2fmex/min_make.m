function [modlist,funlist,fun_info]=min_make(typestrlist)
...
for i=1:length(typestrlist)
 ...
 for m=1:length(typestr)
  temp=any(strcmp(typestr(m),{'d','m'})); if temp,typestr2(m)='c';end
  temp=any(strcmp(typestr(m),{'e','n'})); if temp,typestr2(m)='r';end
  temp=any(strcmp(typestr(m),{'f','o'})); if temp,typestr2(m)='i';end
  temp=any(strcmp(typestr(m),{'g','p'})); if temp,typestr2(m)='l';end
 end
 [modlist,funlist,len]=makeheader(funname,1,typestr2,modlist,funlist,r);
 fun_info{2}='i';
 ...
 for ii=1:min(len,2)
  if any(strcmp(typestr(ii),typs{1}))
   funlist=[funlist,'       real mat',num2str(ii),'(size(in_',num2str(ii),',1),size(in_',num2str(ii),',2))',r];
  elseif any(strcmp(typestr(ii),typs{4}))
   funlist=[funlist,'       real mat',num2str(ii),'(1,size(in_',num2str(ii),'))',r];
  else
 ...
 if length(typestr)==1
  ...
  kernel=[kernel,'         allocate(out(1,1))',r];
  kernel=[kernel,'         temp1=minloc(mat1(1,:))',r];
  if ((typestr(1)=='r')|(typestr(1)=='c')|(typestr(1)=='i'))
  ...
  kernel=[kernel,'          temp1=minloc(mat1(:,i))',r];
  if ((typestr(1)=='r')|(typestr(1)=='c')|(typestr(1)=='i'))
