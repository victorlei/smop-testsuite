function [modlist,funlist,fun_info]=asech_make(typestrlist)
...
for i=1:length(typestrlist)
 ...
 if any(strcmp(typestr,com))
  kernel=[kernel,'        out=log(1.0/in_1+sqrt(-1.0+1.0/in_1**2))',r];
 else
  kernel=[kernel,'        out=real(log(1.0/in_1+sqrt(-1.0+1.0/in_1**2)))',r];
  if any(strcmp(typestr,arr))
   kernel=[kernel,'        if (any((abs(in_1)<=0.0).or.(abs(in_1)>1.0))) then',r];
  else
   kernel=[kernel,'        if ((abs(in_1)<=0.0).or.(abs(in_1)>1.0)) then',r];
  end
  kernel=[kernel,'         call mexPrintf(''Real input to ',funname,' has value <= 0 or > 1: possible errors'')',r];
