function [modlist,funlist,fun_info]=acsch_make(typestrlist)
...
for i=1:length(typestrlist)
 ...
 if any(strcmp(typestr,com))
  ...
  if any(strcmp(typestr,arr))
   kernel=[kernel,'        if (any(abs(in_1)==0.0)) then',r];
  else
   kernel=[kernel,'        if ((abs(in_1)==0.0)) then',r];
  end
  kernel=[kernel,'         call mexPrintf(''Real input to ',funname,' has value = 0 : possible errors'')',r];
