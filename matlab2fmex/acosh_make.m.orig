function [modlist,funlist,fun_info]=acosh_make(typestrlist)
...
for i=1:length(typestrlist)
 ...
 funlist=[funlist,'       real*4 out1',r];
 funlist=[funlist,'       out1=42.2',r];
 
 %And now the kernel %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 kernel='';
 if any(strcmp(typestr,com))
  kernel=[kernel,'        out=log(in_1+sqrt(-1.0+in_1**2))',r];
 else
  kernel=[kernel,'        out=real(log(in_1+sqrt(-1.0+in_1**2)))',r];
  if any(strcmp(typestr,arr))
   kernel=[kernel,'        if (any(abs(in_1)<1.0)) then',r];
  else
   kernel=[kernel,'        if ((abs(in_1)<1.0)) then',r];
  end
  kernel=[kernel,'         call mexPrintf(''Real input to ',funname,' has value < 1 : possible errors'')',r];
  kernel=[kernel,'         print *,''isnan(out)'',isnan(out)',r];
%  kernel=[kernel,'         print *,''hey'',isnanf(out)',r];
